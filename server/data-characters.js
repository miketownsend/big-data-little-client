...
const { parse, format, addYears } = require("date-fns")
...
const classStats = {
  ...
  "Barbarian": { strength: 2, dexterity: 2, constitution: 2, wisdom: -2, intelligence: -2 },
  "Wizard": { intelligence: 2 },
  "Sorceror": { charisma: 2 },
  "Cleric": { strength: 1, constitution: 1, wisdom: 2, dexterity: -1, intelligence: -1 },
...
const raceStats = {
  ...
  "Human": { height: 5, age: 85, strength: 1, constitution: 1, wisdom: 1, dexterity: 1, intelligence: 1 },
  "Gnome": { height: 3, age: 400, dexterity: 2 },
  "Dragonborn": { height: 5, age: 80, strength: 2, charisma: 1 },
  "Lizardfolk": { height: 5, age: 60, strength: 2, constitution: 2, intelligence: -2 }
}
...
const getRaceStat = (race, stat) => raceStats[race][stat] ? raceStats[race][stat] : 0
const getClassStat = (classname, stat) => classStats[classname][stat] ? classStats[classname][stat] : 0
const getRageImage = ({ race }) => 'http://localhost:3003/images/' + faker.random.arrayElement(images[race])
const calculateStat = ({ race, classname }, stat, min=8, max=14) => {
  return faker.random.number({ min, max }) + getRaceStat(race, stat) + getClassStat(classname, stat)
}
...
const calculateHeight = (character) => {
  return `${calculateStat(character, 'height', 0, 1)}'${faker.random.number({
      min: 0,
      max: 11
    })}`
}
...
for (let i = 0; i < 25000; i++) {
  const character = {
    ...
    lair_password: faker.hacker.verb() + ' ' + faker.hacker.verb(),
  ...
  data.push({
    ...character,
    image: getRageImage(character),
    dob: calculateDob(character),
    height: calculateHeight(character),
    strength: calculateStat(character, 'strength'),
    dexterity: calculateStat(character, 'dexterity'),
    constitution: calculateStat(character, 'constitution'),
    intelligence: calculateStat(character, 'intelligence'),
    wisdom: calculateStat(character, 'wisdom'),
    charisma: calculateStat(character, 'charisma'),
  })
}
